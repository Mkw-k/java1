...
//	private int memberCount;
	...
	public BaseballDao() {
		...
		*/
		...
		if(map.isEmpty()) {
			...
			int max = (array[0].getNumber() < 2000) ? array[0].getNumber() : array[0].getNumber()-1000;
			for (int i = 0; i < array.length; i++) {
				
				int num = array[i].getNumber();
				if(array[i].getNumber() > 2000) {
					num = array[i].getNumber() - 1000;
				}
	...
	// 방어율 순위
	public void defenseRanking() {
		// 1.투수				
		TreeMap<String, HumanDto> tMap = new TreeMap<String, HumanDto>();
		
		Map<Integer, HumanDto> pitcherMap = new HashMap<Integer, HumanDto>();		
		Iterator<Integer> it = map.keySet().iterator();
		while(it.hasNext()) {
			int key = it.next();
			HumanDto h = map.get(key);
			if(h.getNumber() < 2000) {	// 투수만
				pitcherMap.put(key, h);
			}			
		}		
		
		// 2.같은 수를 key저장 불가
		// key -> sorting(int, String)
		Iterator<Integer> it1 = pitcherMap.keySet().iterator();
		while(it1.hasNext()) {
			int key = it1.next();
			HumanDto human = pitcherMap.get(key);
			
			String str = ((PitcherDto)human).getDefence() + "";
			// 0.2, 0.2
			HumanDto h = tMap.get(str);
			if(h != null) {
				str = str + "1";
			}			
			tMap.put(str, human);
